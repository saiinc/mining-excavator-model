...
uint8_t I2C::write(uint8_t address, uint8_t registerAddress, uint16_t *data, uint8_t numberBytes)
{
  returnStatus = 0;
  returnStatus = start();
  if(returnStatus){return(returnStatus);}
  ...
  for (uint8_t i = 0; i < numberBytes * 2; i++) {
	sendByte((uint8_t)(data[i] >> 8)); // MSB
    returnStatus =  sendByte((uint8_t)data[i++]); // LSB
    if(returnStatus)
      {
        if(returnStatus == 1){return(3);}
        return(returnStatus);
